...
class EloMonitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    """
    ...
    def __init__(
        ...
        if filename is not None:
            ...
            self.results_writer = EloResultsWriter(
                filename,
...
class EloResultsWriter:
    ...
    def __init__(
        self,
        filename: str = "",
        header: Optional[dict[str, float | str]] = None,
        extra_keys: tuple[str, ...] = (),
        override_existing: bool = True,
    ):
        if header is None:
            header = {}
        if not filename.endswith(EloMonitor.EXT):
            if os.path.isdir(filename):
                filename = os.path.join(filename, EloMonitor.EXT)
            else:
                filename = filename + "." + EloMonitor.EXT
