...
from tl_search.envs.typing import (
    FixedObj,
    InfoDict,
    ObjectId,
    ObsDict,
...
from tl_search.policies.probabilistic import FightPolicy, PatrolPolicy, CapturePolicy
...
class HeuristicEnemyEnv(gym.Env):
    def __init__(
        self,
        ...
        self._enemy_policy_mode: str = enemy_policy_name
        ...
        match enemy_policy_name:
            ...
            case _:
                raise Exception(
                    f"[tl_search] The enemy policy {enemy_policy_name} is not defined."
                )
    ...
    def step(self, action: int) -> tuple[ObsDict, float, bool, bool, InfoDict]:
        ...
        new_blue_agent_loc = Location(
            self._blue_agent_loc.y + direction.y, self._blue_agent_loc.x + direction.x
        )
        match self._is_move_clipped:
            case True:
                ...
                new_blue_agent_loc = Location(
                    np.clip(new_blue_agent_loc.y, 0, num_row - 1),
                    np.clip(new_blue_agent_loc.x, 0, num_col - 1),
                )

                if new_blue_agent_loc in self._fixed_obj.obstacle:
                    pass
                else:
